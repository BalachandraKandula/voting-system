...
def reg_server(root,frame1,name,Gender,zone,city,passw):
    if(passw=='' or passw==' '):
...
def Register(root,frame1):
    ...
    reg = Button(frame1, text="Register", command = lambda: reg_server(root, frame1, name.get(), Gender.get(), zone.get(), city.get(), password.get()), width=5)
    Label(frame1, text="").grid(row = 15,column = 0)
    reg.grid(row = 16, column = 3, columnspan = 2)
